...
then
  ...
  git checkout master > /dev/null 2>&1
  [ `eval git log --pretty="oneline" | grep -io 'New change' | wc -l` -eq 1 ] && echo 'Commit msg check on master... Pass' || echo 'Commit msg check on master... Fail'
  cd ..
  ...
  then
    ...
    [ `eval git branch -r | wc -l` -ge 1 ] && echo 'Branch check... Pass' || echo 'Branch check... Fail'
    git checkout master > /dev/null 2>&1
    [ `eval git log --pretty="oneline" | wc -l` -eq 8 ] && echo 'Commits check on master... Pass' || echo 'Commits check on master... Fail'
    cd ../..
    rm -rf $4/$6
  else
