...
then
  ...
  git checkout master > /dev/null 2>&1
  [ `eval git log --pretty="oneline" | grep -io 'New change' | wc -l` -eq 1 ] && user_score=`expr $user_score + 1`
  cd ..
  ...
  then
    ...
    [ `eval git branch -r | wc -l` -ge 1 ] && user_score=`expr $user_score + 1`
    git checkout master > /dev/null 2>&1
    [ `eval git log --pretty="oneline" | wc -l` -eq 8 ] && user_score=`expr $user_score + 1`
    cd ../..
    rm -rf $4/$6
  fi
fi

percent_score=$(( user_score * 100 / 11 ))
echo "Points scored: $user_score/11"
